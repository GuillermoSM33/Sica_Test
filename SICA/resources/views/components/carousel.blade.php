<div class="carousel-container container-fluid">

    <div id="carouselExampleInterval" class="carousel slide" data-bs-ride="carousel">
        <div class="carousel-inner">
            <div class="carousel-item active img-for-carousel" data-bs-interval="9000">
                <img src="img/CAROUSEL/carousel4.jpg" class="d-block w-100" alt="...">
            </div>
            <div class="carousel-item img-for-carousel" data-bs-interval="9000">
                <img src="img/CAROUSEL/carousel1.jpg" class="d-block w-100" alt="...">
            </div>
            <div class="carousel-item img-for-carousel">
                <img src="img/CAROUSEL/carousel5.jpg" class="d-block w-100" alt="...">
            </div>
        </div>
        <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleInterval" data-bs-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
            <span class="visually-hidden">Previous</span>
        </button>
        <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleInterval" data-bs-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
            <span class="visually-hidden">Next</span>
        </button>
    </div>

</div>
